...
int main(int argc, char **argv) {
  ...
  // いざ、ダンジョンへ
  int winCount = goDungeon(argv[1], /* HINT: ダンジョンのアドレス */&dungeon);

  // 冒険終了後
  if (winCount == /* HINT: ダンジョンのモンスター数 */dungeon.numMonsters) {
    printf("***GAME CLEAR!***\n");
  } else {
    printf("***GAME OVER***\n");
  }
...
// (2)ダンジョン開始から終了までの流れ
int goDungeon(char *playerName, /* ダンジョンのアドレス */Dungeon *pDungeon) {
  printf("%sはダンジョンに到着した\n", playerName);
...
// (3)バトル開始から終了までの流れ
int doBattle(char *playerName, /* 戦う敵モンスターのアドレス */Monster *pEnemy) {
...
// (A)モンスター名のカラー表示
void printMonsterName(/* モンスターのアドレス */Monster *m) {
  // HINT: char型変数symbolを宣言し、引数で指定されたモンスターの
  //       属性記号($や#など）を取得する。
  //       （ELEMENT_SYMBOLS配列から取り出す）
  char symbol = ELEMENT_SYMBOLS[m->element];
  ...
  printf("\x1b[3%dm", ELEMENT_COLORS[m->element]);
